...
access, and those just delayed but still served, (3) which named hospitals \
absorb the lost access or lose catchment population, (4) one concrete \
recommendation. Use bullet points for the numbers. End with a one-line \
"Bottom line".
...
in one line, (2) rank the tested interventions from most to least access \
restored, (3) explain why the top intervention helps the most (e.g. it \
reconnects the most high-impact zones), (4) give one action recommendation. \
Use bullet points for the numbers. End with a one-line "Bottom line".
...
def explain_interventions(impact: dict, interventions: list, net) -> dict:
    ...
    for i, r in enumerate(interventions, 1):
        rows.append(
            ...
            f"accessibility recovery {r['accessibility_recovery_pct']:.1f}%"
        )
    ...
    if text is None:
        ...
        lines = [
            ...
            f"- accessibility recovery {best['accessibility_recovery_pct']:.1f}%.",
...
def _template_summary(impact: dict, net, interventions: Optional[list] = None) -> str:
    ...
    if interventions:
        ...
        lines.append(
            ...
            f"{best['population_restored']:,} people and recovers "
            f"{best['accessibility_recovery_pct']:.1f}% of accessibility."
        )
    else:
        lines.append(
